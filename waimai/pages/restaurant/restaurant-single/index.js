...
Page({
    ...
    onShow: function () {
        var that = this;
        that.setData({ submitIsLoading: false, this_search_isshow:false});
        let rdata = {};
        rdata.dish_id = that.data.this_dish_id;
        rdata.order_type = that.data.this_order_type;
        requestUtil.get(_DuoguanData.duoguan_host_api_url + '/index.php?s=/addon/DuoguanDish/ApiSingle/getDishInfo.html', rdata, (info) => {
            that.setData({
                dish_data: info,
    ...
    get_goods_list_bycid: function (cate_id, is_loading = 1) {
        ...
        requestUtil.get(_DuoguanData.duoguan_host_api_url + '/index.php?s=/addon/DuoguanDish/ApiSingle/getGoodsListByCateId.html', requestData, (info) => {
            ...
            that.setData({ this_cate_data_goods: info, this_cate_data_id: cate_id, this_search_key:''});
