...
module.exports = {
    ...
    getRequestOptions: function (options, page) {
                ...
                try {
                    ...
                    if (code == 1) {
                        ...
                        if (options.loginInvalidCount > 1) {
                          wx.showModal({ content: 'Please try again later~', showCancel: false });
                            return;
                        }
        ...
        options.loadingText = options.loadingText || "Please wait";
    ...
    upload: function (url, filepath, name, data, callback, page, options) {
      wx.showToast({ title: 'Uploading...', icon: 'loading', duration: 10000 });
        options = $.extend({
                    ...
                    setTimeout(function () {
                        if (newToken == null) {
                            ...
                            options.completeAfter && options.completeAfter.apply(page,
                              [{ statusCode: 0, errMsg: "request timeout", data: null }]);
                        }
                    }, 10000);
                } else {
    ...
    login: function (obj) {
        ...
        isLoginLoading = true;
        wx.showToast({ title: 'Landfall', icon: 'loading', duration: 10000, });
        wx.login({
            success: function (res) {
