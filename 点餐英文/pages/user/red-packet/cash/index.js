...
Page({
    ...
    initialize: function (options) {
        ...
        request.get(requestUrl, requestData, (info) => {
            ...
            if (this.data.is_show_confirm) {
              dg.alert(this.data.confirm_content, function () { }, 'Reminder');
            }
    ...
    chooseAccount: function (e) {
        ...
        if (this.data.is_use_bank == false && this.data.is_use_app_balance == false) {
          dg.alert("Not set up information in the background");
        } else if (this.data.is_use_bank == true && this.data.is_use_app_balance == true) {
          let app_name = "WeChat balance";
          if (isAli) app_name = "Alipay balance";
            ...
            if (isAli) { // 支付宝
                my.showActionSheet({
                  items: ['Please choose the mode of presentation', app_name, 'Bank card'],
                ...
                wx.showActionSheet({
                  itemList: ['Please choose the mode of presentation', app_name, 'Bank card'],
                    success: function (res) {
                        if (res.tapIndex == 1) {
                            that.setData({ cash_type: 1 });
                            that.writeMobile(that);
